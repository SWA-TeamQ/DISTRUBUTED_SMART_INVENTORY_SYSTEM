...
18. As a **User**, I want the full-resolution primary image to load in the background and replace the thumbnail, so that I see a crisp image without blocking the UI.
19. As a **User**, I want to click small thumbnail previews to load additional images (up to 3 total) on demand, so that I only download what I need.

### Bidding

20. As a **User**, I want to place a bid on an active auction, so that I can compete for items.
21. As a **User**, I want the system to enforce a minimum bid increment of 5% above the current price, so that penny-sniping is prevented.
22. As a **User**, I want the system to reject my bid if someone else bid in the meantime (stale price detection), so that I don't accidentally underbid.
...
31. As a **User**, I want to see "My Activity" with tabs for My Bids, Won, and Outbid, so that I can track my bidding history.
...
48. As a **User**, I want to download a CSV file containing: AuctionID, Title, Category, StartingPrice, FinalPrice, Winner, Status, StartTime, EndTime — so that I have a complete offline record.
...
50. As a **User**, I want to see a clear error alert if the server is unreachable, so that I know the connection is down.
51. As a **User**, I want the polling thread to stop automatically when I close the auction detail view, so that resources are cleaned up.
...
```
[2026-05-09T14:30:00Z] [INFO] admin: Created user 'user1' with role USER
[2026-05-09T14:31:15Z] [INFO] user1: Created auction #5 'Vintage Guitar'
[2026-05-09T14:32:00Z] [INFO] user2: Placed bid 15000 on auction #5
[2026-05-09T14:32:28Z] [INFO] REAPER: Auction #3 expired (no bids)
[2026-05-09T14:32:28Z] [INFO] REAPER: Auction #4 sold to user3 for 32000
[2026-05-09T14:33:00Z] [WARN] user2: Bid rejected on auction #5 (stale price)
```

### 12. JavaFX Views & Application Flow

```
App Start → Connect → Login → {Admin Panel | Gallery | User Dashboard}
```
