...
public class AuctionManager {
  ...
  public List<AuctionItem> findActiveAuctionsBySeller(String sellerUsername) {
    return auctionRepo
      .findAuctionsBySeller(sellerUsername)
      .stream()
  ...
  public void cancelAuction(int auctionId, SessionContext user)
    throws Exception {
    lockManager.lock(auctionId);
    try {
      txManager.executeWithoutResult(() -> {
        ...
        if (
          !item.getSellerUsername().equals(user.username()) &&
          !Constants.ADMIN.equals(user.role())
        ) {
          throw new AuctionException(
            "Only the seller or an admin can cancel this auction"
          );
        }
  ...
  public int relistAuction(
    ...
    try {
      return txManager.execute(() -> {
        ...
        if (parent == null) throw new AuctionException(
          "Parent auction not found"
        );

        if (
          !parent.getSellerUsername().equals(user.username()) &&
          !Constants.ADMIN.equals(user.role())
        ) {
          throw new AuctionException(
            "Only the seller or an admin can relist this auction"
          );
        }

        if (
          !Constants.STATUS_EXPIRED.equals(parent.getStatus()) &&
          !Constants.STATUS_CANCELLED.equals(parent.getStatus())
        ) {
          throw new AuctionException(
            "Only expired or cancelled auctions can be relisted"
          );
        }
  ...
  private void validateNotSeller(AuctionItem item, String bidder)
    throws SelfBidException {
    if (bidder.equals(item.getSellerUsername())) {
      throw new SelfBidException("You cannot bid on your own auction.");
    }
  }

  private void validateNotCurrentWinner(AuctionItem item, String bidder)
  ...
  private void validateFreshness(AuctionItem item, long expectedCents)
  ...
  private void validateMinimumBid(AuctionItem item, long amountCents)
    throws InsufficientBidException {
    if (amountCents <= 0) {
    ...
    if (item.getHighestBidderUsername() == null) {
      ...
      long minBidCents =
        item.getCurrentBidCents() + (item.getCurrentBidCents() + 19) / 20;
  ...
  private void validateNotExpired(AuctionItem item)
