...
public interface IAuctionService extends Remote {
   ...
   */
  default List<AuctionItem> searchActiveAuctions(
    String query,
    String category,
    String sortBy
    ...
    if (!q.isBlank()) {
      stream = stream.filter(
        a ->
          containsIgnoreCase(a.getTitle(), q) ||
    ...
    if ("price_asc".equals(sortBy)) {
      return filtered
        .stream()
        .sorted(
          java.util.Comparator.comparingLong(AuctionItem::getCurrentBidCents)
        )
        .toList();
    }
      ...
      return filtered
        ...
        .sorted(
          java.util.Comparator.comparingLong(
            AuctionItem::getCurrentBidCents
          ).reversed()
        )
        .toList();
    }
   ...
   */
  default List<AuctionItem> searchAllAuctions(
    String query,
    String category,
    String sortBy
  ) throws RemoteException {
  ...
  List<AuctionItem> getActiveAuctionsBySeller(
  ...
  default void updateAuction(
    ...
    throw new AuctionException(
      "Auction editing is not supported by this implementation"
    );
  }
  ...
  void startAuction(int auctionId, String token)
    throws RemoteException, AuctionException;

  // --- Bidder Activity ---
  List<Bid> getMyBids(String token) throws RemoteException, AuctionException;
  List<AuctionItem> getMyWonAuctions(String token)
    throws RemoteException, AuctionException;
  ...
  // --- Data Export ---
  byte[] exportAuctionsToCSV(String token)
    throws RemoteException, AuctionException;
